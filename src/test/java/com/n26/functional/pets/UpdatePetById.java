...
public class UpdatePetById extends BaseTest{
    ...
    public void testValidPetUpdateWithNameStatus()
    {
        ...
        Log.scriptInfo("old pet name "+respsonse.jsonPath().getString("name")+" is successfully updated with new name as : "+postRespsonse.jsonPath().getString("name"));
        Log.scriptInfo("old pet status "+respsonse.jsonPath().getString("status")+" is successfully updated with new status as : "+postRespsonse.jsonPath().getString("status"));
        ...
        //getting updated pet details by id
          ...
          Assert.assertEquals(getResponse.jsonPath().getString("name").equals(getResponse.jsonPath().getString("name")),true);
          Assert.assertEquals(getResponse.jsonPath().getString("status").equals(getResponse.jsonPath().getString("status")),true);
